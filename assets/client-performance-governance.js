(() => {
  ...
  async function enhanceKpiForm(){
    ...
    const holder=document.createElement('div');holder.className='perf-wide';holder.id='pkGovernance';holder.innerHTML=`<div class="perf-note"><b>Governança VOS</b> · defina o papel do indicador e em que etapa da jornada ele acontece. A ordem técnica é calculada automaticamente.</div><div class="perf-formgrid" style="margin-top:0"><div><label class="perf-label">Papel do KPI</label><select id="pkVosRole" class="perf-select"><option value="PRINCIPAL" ${kpi.vos_role==='PRINCIPAL'?'selected':''}>Principal — classifica o resultado do cliente</option><option value="SECUNDARIO" ${!kpi.vos_role||kpi.vos_role==='SECUNDARIO'?'selected':''}>Secundário — ajuda a explicar a evolução</option><option value="CONTEXTO" ${kpi.vos_role==='CONTEXTO'?'selected':''}>Contexto — não classifica resultado</option></select></div><div><label class="perf-label">Etapa da jornada</label><select id="pkJourneyStage" class="perf-select"><option value="" ${!kpi.journey_stage?'selected':''}>Selecione…</option><option value="EXPOSICAO" ${kpi.journey_stage==='EXPOSICAO'?'selected':''}>Exposição — a pessoa viu</option><option value="INTERESSE" ${kpi.journey_stage==='INTERESSE'?'selected':''}>Interesse — demonstrou curiosidade</option><option value="ACAO" ${kpi.journey_stage==='ACAO'?'selected':''}>Ação — deu um passo</option><option value="OPORTUNIDADE" ${kpi.journey_stage==='OPORTUNIDADE'?'selected':''}>Oportunidade — entrou no comercial</option><option value="RESULTADO" ${kpi.journey_stage==='RESULTADO'?'selected':''}>Resultado — compra, receita ou resultado final</option></select><small style="display:block;font-size:12px;color:#6b7c91;margin-top:5px">Você não precisa atribuir número de prioridade.</small></div><div><label class="perf-label">Mudança relevante (%)</label><input id="pkTolerance" class="perf-input" type="number" min="0" max="100" step="0.1" value="${E(kpi.meaningful_change_pct??5)}"></div><div><label class="perf-label">Janela padrão de avaliação (dias)</label><input id="pkEvalWindow" class="perf-input" type="number" min="1" max="365" value="${E(kpi.evaluation_window_days||30)}"></div></div><div style="margin-top:9px">${kpi.baseline_locked_at?`<span class="perf-badge ok">Baseline travado em ${new Date(kpi.baseline_locked_at).toLocaleDateString('pt-BR')}</span><div style="font-size:12px;color:#6b7c91;margin-top:5px">O valor e o período não podem mais ser sobrescritos.</div>`:`<label><input id="pkLockBaseline" type="checkbox"> <span style="font-size:13px;font-weight:700;color:#0a2540">Confirmo que este baseline foi validado e quero travá-lo após salvar.</span></label>`}</div>`;
    grid.append(holder);
  ...
  document.addEventListener('click',async(event)=>{
    ...
    try{
      ...
      if(!q('pkJourneyStage').value){YM.toast('Escolha em qual etapa da jornada este KPI acontece.',true);btn.disabled=false;return;}
      ...
      await semantic({action:'UPDATE_META',client_id:clientId,kpi_id:kpiId,vos_role:q('pkVosRole').value,journey_stage:q('pkJourneyStage').value,meaningful_change_pct:Number(q('pkTolerance').value||0),evaluation_window_days:Number(q('pkEvalWindow').value||30)});
