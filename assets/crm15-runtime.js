(()=>{
  ...
  window.updateKpis=function(items){const raw=document.getElementById('totalChip')?.textContent||'',parsed=Number((raw.match(/\d+/)||[])[0]||0),totalAll=Math.max(parsed,items.length,1),pct=n=>`${((n/totalAll)*100).toFixed(1).replace('.',',')}% do total`;const leads=items.filter(o=>o.current_stage==='LEAD_MAPEADO').length,read=items.filter(window.readingSent).length,rx=items.filter(window.rxDelivered).length,w=items.filter(window.won).length,set=(id,v)=>{const e=document.getElementById(id);if(e)e.textContent=v};set('kTotal',items.length);set('kLead',leads);set('kReading',read);set('kRx',rx);set('kWon',w);set('pLead',pct(leads));set('pReading',pct(read));set('pRx',pct(rx));set('pWon',pct(w))};
  function loadClientsRuntime(){if(document.querySelector('script[data-ym-clients]'))return;const s=document.createElement('script');s.src='/assets/crm15-clients.js?v=20260812-1';s.defer=true;s.dataset.ymClients='1';document.head.append(s)}
  loadClientsRuntime();
