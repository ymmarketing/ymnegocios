...
Deno.serve(async (req) => {
  ...
  let input: { renewalMode?: string; email?: string } = {};
  try { input = await req.json(); } catch { /* default */ }

  const email = normalizeEmail(authenticatedUser?.email || input.email);
  ...
  pendingQuery = userId
    ...
    : pendingQuery.is('user_id', null).eq('purchaser_email', email);
  ...
  if (!userId) {
    const { data: unclaimed } = await admin
      ...
      .eq('status', 'paid_unclaimed')
      .limit(1)
      .maybeSingle();
    if (unclaimed) return json({ ok: false, error: 'access_already_paid_create_account' }, 409);
  } else {
    const { data: liveAutoRenew } = await admin
      ...
      .limit(1)
      .maybeSingle();
    if (liveAutoRenew) return json({ ok: false, error: 'auto_renew_already_active' }, 409);
    ...
    const { data: scheduledJourney } = await admin
      ...
      .limit(1)
      .maybeSingle();
    if (scheduledJourney) return json({ ok: false, error: 'renewal_already_paid' }, 409);
